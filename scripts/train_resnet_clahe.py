...
BATCH_SIZE = 16
EPOCHS = 25              # ResNet converges fast
LEARNING_RATE = 0.0001
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")

print(f"⚡ Initializing ResNet-50 (CLAHE Edition) on: {DEVICE}")
...
for epoch in range(EPOCHS):
    ...
    # Save the best ResNet model for the new ensemble
    if test_acc > best_acc:
        best_acc = test_acc
        torch.save(model.state_dict(), "resnet50_clahe_BEST.pth")

print(f"✅ ResNet-50 (CLAHE) Training Complete. Best Test Accuracy: {best_acc:.2f}%")
